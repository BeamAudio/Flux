...
public:
     ...
     */
    void setAudioCallback(std::function<void(const std::map<std::string, float*>&, float**, int, int, int, double)> callback);
    ...
    /**
     * @brief Opens an additional input device stream.
     */
    SDL_AudioStream* openInputStream(const std::string& deviceId);

    /**
     * @brief Returns all active input streams.
     */
    const std::map<std::string, SDL_AudioStream*>& getActiveInputStreams() const { return m_inputStreams; }
...
private:
    ...
    std::function<void(const std::map<std::string, float*>&, float**, int, int, int, double)> m_audioCallback;

    SDL_AudioStream* m_outputStream = nullptr;
    std::map<std::string, SDL_AudioStream*> m_inputStreams;
};
