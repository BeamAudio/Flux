...
public:
    FluxPluginProcessor(float sampleRate = 44100.0f, std::shared_ptr<MeterSource> meterSource = nullptr) 
        : m_sampleRate(sampleRate), m_meterSource(meterSource) {}
    virtual ~FluxPluginProcessor() = default;

    void releaseNode() override { delete this; }
...
public:
    ...
    // Optional hook for plugins to update their parameters from processor metrics
