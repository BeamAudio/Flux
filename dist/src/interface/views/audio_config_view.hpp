...
public:
    AudioConfigView(class BeamHost* host, AudioDeviceManager* manager, AudioEngine* engine) 
        ...
        m_addPathBtn = std::make_shared<TextButton>("ADD PATH");
        m_rescanBtn = std::make_shared<TextButton>("RE-SCAN PLUGINS");

        addChildComponent(m_sampleRateCombo);
        addChildComponent(m_bufferSizeCombo);
        addChildComponent(m_addPathBtn);
        addChildComponent(m_rescanBtn);
    ...
    void setupCallbacks() {
        m_addPathBtn->onClick([this]() {
            SDL_ShowOpenFileDialog(onAddPathCallback, this, nullptr, nullptr, 0, nullptr, false);
        });
        m_rescanBtn->onClick([this]() {
            for (const auto& path : PluginLibrary::get().getScanPaths()) {
                if (path != "plugins") FluxCompiler::scanVST3(path);
            }
        });
    }

    static void onAddPathCallback(void* userdata, const char* const* filelist, int filter) {
        if (filelist && filelist[0]) {
            AudioConfigView* view = static_cast<AudioConfigView*>(userdata);
            std::string folder = filelist[0];
            // Remove filename if any (SDL ShowOpenFileDialog on folders is tricky, 
            // usually you select a file in the folder or it's a folder picker depending on OS)
            PluginLibrary::get().addScanPath(folder);
        }
    }
    ...
    void paint(QuadBatcher& batcher) override {
        ...
        drawLabel("VST Scan Paths:", yOff + 120);

        // List paths
        float py = yOff + 150;
        for (const auto& path : PluginLibrary::get().getScanPaths()) {
            batcher.drawText(path, xOff, py, 10, 0.7f, 0.7f, 0.75f, 1.0f);
            py += 15;
        }
    ...
    void resized() override {
        ...
        m_addPathBtn->setBounds(winX + winW - 130, winY + winH - 80, 100, 24);
        m_rescanBtn->setBounds(winX + winW - 180, winY + winH - 45, 150, 24);
    }
...
private:
    ...
    std::shared_ptr<TextButton> m_addPathBtn;
    std::shared_ptr<TextButton> m_rescanBtn;
