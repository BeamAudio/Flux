                              ...
                              float r, float g, float b, float a) 
{
    ...
    // Note: This basic batcher doesn't support multi-texturing yet.
    // Ideally we would check m_currentTexture != textureId and flush.
    // For now, assume users flush manually or we flush on texture switch if we track it.
    
    // Simple state change handling:
    flush(); 
    if (m_shader) {
        m_shader->setInt("mode", 1); // Texture mode
        glBindTexture(GL_TEXTURE_2D, textureId);
    }

    m_vertices.push_back({{x, y}, {u0, v0}, {r, g, b, a}});
    m_vertices.push_back({{x + w, y}, {u1, v0}, {r, g, b, a}});
    m_vertices.push_back({{x + w, y + h}, {u1, v1}, {r, g, b, a}});
    m_vertices.push_back({{x, y + h}, {u0, v1}, {r, g, b, a}});

    m_quadCount++;
    flush(); // Immediate flush to handle state change safely for now
    
    if (m_shader) {
        m_shader->setInt("mode", 0); // Revert to solid
        glBindTexture(GL_TEXTURE_2D, 0);
    }
}
...
void QuadBatcher::drawSmoothLine(float x1, float y1, float x2, float y2, float thickness, float r, float g, float b, float a) {
    ...
    if (m_shader) {
        ...
        m_shader->setFloat("uEdgeSoftness", 0.1f); // Adjust for smoothness
    }
    ...
    // We use TexCoord.y to represent the distance across the line width [0, 1]
