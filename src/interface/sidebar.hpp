...
public:
    ...
    void render(QuadBatcher& batcher) override {
        ...
        if (m_side == Side::Left) {
            ...
            auto drawBtn = [&](const std::string& label) {
                batcher.drawQuad(m_bounds.x + 10, yOff, m_bounds.w - 20, 30, 0.15f, 0.16f, 0.17f, 1.0f);
                batcher.drawText(label, m_bounds.x + 20, yOff + 8, 14, 0.7f, 0.7f, 0.7f, 1.0f);
                yOff += 40;
            };
            drawBtn("GAIN");
            drawBtn("FILTER");
    ...
    bool onMouseDown(float x, float y, int button) override {
        if (m_side == Side::Left) {
            float yOff = m_bounds.y + 50;
            if (y > yOff && y < yOff + 30) { if (onAddFX) onAddFX("Gain"); return true; } yOff += 40;
            if (y > yOff && y < yOff + 30) { if (onAddFX) onAddFX("Filter"); return true; } yOff += 40;
            if (y > yOff && y < yOff + 30) { if (onAddFX) onAddFX("Delay"); return true; } yOff += 40;
            if (y > yOff && y < yOff + 30) { if (onAddFX) onAddFX("Tube"); return true; }
        }
        return false;
    }
