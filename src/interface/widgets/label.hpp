...
public:
    ...
    // Using simple struct for now to avoid Theme dependency if not needed, 
    // but sidebar passed a brace-init list {r,g,b,a}, so let's match that.
    struct LabelColor { float r, g, b, a; };
    void setColor(LabelColor c) { m_textColor = c; }
    // Overload for individual floats
    void setColor(float r, float g, float b, float a) { m_textColor = {r, g, b, a}; }
    ...
    void getPreferredSize(float& w, float& h) const override {
        // Approximate width based on name and font size
        w = m_text.length() * (m_fontSize * 0.6f); 
        h = m_fontSize + 4.0f;
    }

    void paint(QuadBatcher& g) override {
        // Center text vertically in label height (using local coordinates)
...
private:
    ...
    LabelColor m_textColor = {0.9f, 0.9f, 0.9f, 1.0f};
};
