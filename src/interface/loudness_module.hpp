...
public:
    ...
    void render(QuadBatcher& batcher, float dt, float screenW, float screenH) override {
        ...
        if (pM) drawBar("M", pM->getValue(), graphX + 10, 30, 0.13f, 0.62f, 0.42f); // Emerald
        if (pS) drawBar("S", pS->getValue(), graphX + 50, 30, 0.13f, 0.62f, 0.42f); // Emerald
        if (pT) drawBar("TP", pT->getValue(), graphX + 90, 30, 0.56f, 0.03f, 0.03f); // Red
    }
};
