...
public:
    ...
    RackUnitUI(FluxNode* node, RackStyle style) : m_node(node), m_style(style) {
        ...
        cfg.gap = 14.0f; // More horizontal breathing room
        cfg.padding = 8.0f;
        cfg.preferredWidth = 300.0f; 
        cfg.wrap = true;
    ...
    void paint(QuadBatcher& batcher) override {
        ...
        if (m_style.showMeter && m_node->getMeterSource()->getNumMeters() > 0) {
            ...
            int numSegs = 14; // More segments for better resolution
            float gap = 2.0f;
            float segH = (meterH - (numSegs-1)*gap) / numSegs;
            for (int i = 0; i < numSegs; ++i) {
                float segPos;
                if (m_style.invertMeter) {
                    segPos = (float)(numSegs - 1 - i) / (float)(numSegs - 1);
                } else {
                    segPos = (float)i / (float)(numSegs - 1);
                }
                ...
                Color col;
                if (m_style.invertMeter) {
                    // Level Meter: Green at bottom, Red at top
                    col = (segPos < 0.7f) ? Theme::LEDGreen : (segPos < 0.9f ? Theme::LEDYellow : Theme::LEDRed);
                } else {
                    // GR Meter: Green at top, Red at bottom
