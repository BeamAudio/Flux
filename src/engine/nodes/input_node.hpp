...
public:
    InputProcessor(std::shared_ptr<CaptureBuffer> buffer, const std::string& deviceId) 
        : m_buffer(buffer), m_deviceId(deviceId) {}

    void process(const float** inputs, float** outputs, int frames) override {
        float* out = outputs[0];
        float currentPeak = 0.0f;
        
        // This processor now expects data to have been pushed into m_buffer
        // by the AudioEngine during the audio callback.
        
        std::lock_guard<std::mutex> lock(m_buffer->mutex);
        
        if (m_buffer->data.size() >= (size_t)(frames * 2)) {
            for(int i = 0; i < frames * 2; ++i) {
                float s = m_buffer->data[i];
...
public:
    ...
    std::unique_ptr<FluxProcessor> createProcessor() override {
        return std::make_unique<InputProcessor>(m_captureBuffer, m_deviceId);
    }
